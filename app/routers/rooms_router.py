...
def update_room(room_id: str, payload: RoomUpdate, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    return RoomService(db).update_room(room_id, payload)


@router.delete("/{room_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_room(room_id: str, db: Session = Depends(get_db), _: User = Depends(require_admin)):
